# morning_trend_reclaim_two_sided_1200 stop_widen_rescue_1

Parameter-only rescue for `morning_trend_reclaim_two_sided_1200` after the original staged validation failed. Entry mechanics, signal window, VWAP setup mode, carried pivot filter, data windows, costs, and target grid are unchanged. The stop grid changes from `[0.0015, 0.0025, 0.004]` to `[0.0025, 0.004, 0.006]`, and the fixed-config stop changes from `0.0025` to `0.004`.
