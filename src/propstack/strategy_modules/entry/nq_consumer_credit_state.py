...
class NqConsumerCreditStateEntry:
    name = "nq_consumer_credit_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "total_credit_3m_contraction_short")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_consumer_credit_state_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "mian_sufi_household_leverage_fed_g19_consumer_credit",
            ...
            "availability_rule": "latest monthly FRED/Federal Reserve consumer-credit observation on or before session_date minus 60 calendar days",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "consumer_credit_driver_column": driver_column,
            "consumer_credit_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "total_credit_change_3m": row.get("total_credit_change_3m"),
            "revolving_credit_change_1m": row.get("revolving_credit_change_1m"),
            "revolving_credit_change_3m": row.get("revolving_credit_change_3m"),
            "nonrevolving_credit_change_3m": row.get("nonrevolving_credit_change_3m"),
            "revolving_credit_share": row.get("revolving_credit_share"),
            "total_credit_to_income": row.get("total_credit_to_income"),
            "revolving_credit_to_income": row.get("revolving_credit_to_income"),
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_consumer_credit_state_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "total_credit_3m_contraction_short":
            column = "total_credit_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "revolving_credit_3m_relief_long":
            column = "revolving_credit_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "total_credit_to_income_low_long":
            column = "total_credit_to_income_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "revolving_credit_to_income_low_long":
            column = "revolving_credit_to_income_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "revolving_credit_1m_relief_long":
            column = "revolving_credit_change_1m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        raise ValueError(f"Unsupported setup_mode for nq_consumer_credit_state: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ consumer-credit feature CSV not found: {path}")
