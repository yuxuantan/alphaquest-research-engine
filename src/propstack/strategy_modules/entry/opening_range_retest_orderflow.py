...
class OpeningRangeRetestOrderflowEntry(OpeningRangeBreakoutEntry):
    name = "opening_range_retest_orderflow"
    ...
    def __init__(self, params: dict):
        ...
        self.breakout_buffer = self.tick_size * int(params.get("breakout_buffer_ticks", 1))
        self.retest_tolerance = self.tick_size * int(params.get("retest_tolerance_ticks", 4))
        self.hold_buffer = self.tick_size * int(params.get("hold_buffer_ticks", 0))
        self.max_retest_bars = max(1, int(params.get("max_retest_bars", 3)))
        ...
        if self.flow_confirmation not in {"absorbed", "aligned"}:
            raise ValueError("entry.params.flow_confirmation must be 'absorbed' or 'aligned'.")

    def _state(self, session_date):
        state = super()._state(session_date)
        state.setdefault("recent_bars", [])
        state.setdefault("breakout", None)
        return state
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        signal = self._retest_signal(bar, state)
        if signal is not None:
            state["completed"] = True
            return signal
        return None

    def _retest_signal(self, bar: pd.Series, state: dict) -> Signal | None:
        ...
        if close is None or high is None or low is None or opening_range is None:
            return None

        breakout = state.get("breakout")
        if breakout is None:
            state["breakout"] = self._new_breakout_candidate(bar, opening_range, close)
            return None

        breakout["bars_since_breakout"] += 1
        breakout["retest_high"] = max(breakout["retest_high"], high)
        breakout["retest_low"] = min(breakout["retest_low"], low)
        if breakout["bars_since_breakout"] > self.max_retest_bars:
            state["breakout"] = self._new_breakout_candidate(bar, opening_range, close)
            return None
        ...
        if direction == "long":
            failed = close < level - self.hold_buffer
            holds = low <= level + self.retest_tolerance and close >= level + self.hold_buffer
            level_type = "opening_range_high_retest_orderflow"
        else:
            failed = close > level + self.hold_buffer
            holds = high >= level - self.retest_tolerance and close <= level - self.hold_buffer
            level_type = "opening_range_low_retest_orderflow"
        ...
        confirmation_high = max(float(item["high"]) for item in state["recent_bars"]) if state["recent_bars"] else high
        confirmation_low = min(float(item["low"]) for item in state["recent_bars"]) if state["recent_bars"] else low
        trigger_side = "upside" if direction == "long" else "downside"
        return Signal(
            direction=direction,
            level_type=level_type,
            swept_level=level,
            sweep_timestamp=breakout["breakout_timestamp"],
            sweep_high=breakout["breakout_high"],
            sweep_low=breakout["breakout_low"],
            ...
            metadata={
                ...
                "breakout_timestamp": breakout["breakout_timestamp"],
                "trigger_side": trigger_side,
                "bars_to_retest": breakout["bars_since_breakout"],
                "retest_timestamp": confirmation_end,
                ...
                "retest_signed_volume": signed_volume,
                "retest_flow_volume": total_volume,
                "retest_orderflow_imbalance": imbalance,
            },
            report_fields={
                ...
                "retest_flow_volume": total_volume,
                "retest_orderflow_imbalance": imbalance,
                "signal_timestamp": confirmation_end,
                "intended_entry_timestamp": confirmation_end,
            },
        )

    def _new_breakout_candidate(self, bar: pd.Series, opening_range: dict, close: float) -> dict | None:
