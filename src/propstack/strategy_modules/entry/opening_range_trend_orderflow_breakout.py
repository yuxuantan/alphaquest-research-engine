...
class OpeningRangeTrendOrderflowBreakoutEntry(OpeningRangeOrderflowBreakoutEntry):
    name = "opening_range_trend_orderflow_breakout"
    ...
    def _confirmation_signal(
        ...
        trend = self._trend_context(bar, signal.direction)
        if trend is None:
            return None
    ...
    def _trend_context(self, bar: pd.Series, direction: str) -> dict | None:
        state = self.state_by_day.get(bar["session_date"]) or {}
        bars = state.get("all_bars") or []
        if len(bars) < self.long_trend_bars:
            return None

        short_bars = bars[-self.short_trend_bars :]
        long_bars = bars[-self.long_trend_bars :]
        short_context = _trend_window(short_bars, direction, self.tick_size, self.min_trend_move_ticks)
        long_context = _trend_window(long_bars, direction, self.tick_size, self.min_trend_move_ticks)
        if short_context is None or long_context is None:
            return None

        return {
            "trend_filter": "hh_hl_lh_ll",
            ...
            "trend_filter_signal_timestamp": pd.Timestamp(bar["timestamp"])
            + pd.Timedelta(minutes=float(self.params.get("bar_interval_minutes", 1))),
        }
