...
class DefaultSpreadOrderflowStateEntry:
    name = "default_spread_orderflow_state"
    ...
    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "high_spread_long")).lower()
        self.flow_mode = str(params.get("flow_mode", "signed_volume")).lower()
        self.feature_csv = str(
            params.get("feature_csv", "data/external/es_default_spread_features_20110103_20260609.csv")
        )
        ...
        self.credit_rank_threshold = float(params.get("credit_rank_threshold", 0.60))
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "fama_french_1989_default_spread_cont_2014_orderflow",
            "setup_mode": self.setup_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "credit_asof_date": row.get("credit_asof_date"),
            "credit_observation_date": row.get("observation_date"),
            "availability_rule": "latest FRED Aaa/Baa yield observation on or before session_date_minus_2_business_days",
            "credit_driver_column": driver_column,
            "credit_driver_value": driver_value,
            "credit_rank_threshold": self.credit_rank_threshold,
            "aaa_yield": row.get("aaa_yield"),
            "baa_yield": row.get("baa_yield"),
            "default_spread": row.get("default_spread"),
            "default_spread_change_1d": row.get("default_spread_change_1d"),
            "default_spread_rank_252": row.get("default_spread_rank_252"),
            "default_spread_change_1d_rank_252": row.get("default_spread_change_1d_rank_252"),
        ...
        return Signal(
            direction=direction,
            level_type=f"default_spread_orderflow_{self.setup_mode}_{direction}",
    ...
    def _direction_from_features(self, row: dict, state: dict) -> tuple[str | None, str, float]:
        ...
        high_tail = self.credit_rank_threshold
        low_tail = 1.0 - self.credit_rank_threshold
        if self.setup_mode == "high_spread_long":
            rank = _finite_float(row.get("default_spread_rank_252"))
            if rank is not None and rank >= high_tail and move_ticks >= self.min_es_move_ticks:
                return "long", "default_spread_rank_252", rank
            return None, "default_spread_rank_252", rank if rank is not None else float("nan")
        if self.setup_mode == "widening_spread_short":
            rank = _finite_float(row.get("default_spread_change_1d_rank_252"))
            if rank is not None and rank >= high_tail and move_ticks <= -self.min_es_move_ticks:
                return "short", "default_spread_change_1d_rank_252", rank
            return None, "default_spread_change_1d_rank_252", rank if rank is not None else float("nan")
        if self.setup_mode == "tightening_spread_long":
            rank = _finite_float(row.get("default_spread_change_1d_rank_252"))
            if rank is not None and rank <= low_tail and move_ticks >= self.min_es_move_ticks:
                return "long", "default_spread_change_1d_rank_252", rank
            return None, "default_spread_change_1d_rank_252", rank if rank is not None else float("nan")
        if self.setup_mode == "spread_change_two_sided":
            rank = _finite_float(row.get("default_spread_change_1d_rank_252"))
            if rank is None:
                return None, "default_spread_change_1d_rank_252", float("nan")
            if rank <= low_tail and move_ticks >= self.min_es_move_ticks:
                return "long", "default_spread_change_1d_rank_252", rank
            if rank >= high_tail and move_ticks <= -self.min_es_move_ticks:
                return "short", "default_spread_change_1d_rank_252", rank
            return None, "default_spread_change_1d_rank_252", rank
    ...
    def _validate(self) -> None:
        if self.setup_mode not in {
            "high_spread_long",
            "widening_spread_short",
            "tightening_spread_long",
            "spread_change_two_sided",
        }:
        ...
        if not 0.5 <= self.credit_rank_threshold < 1.0:
            raise ValueError("entry.params.credit_rank_threshold must be in [0.5, 1.0).")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"Default-spread feature CSV not found: {path}")
    out: dict[date, dict[str, float | str]] = {}
    string_columns = {"credit_asof_date", "observation_date"}
