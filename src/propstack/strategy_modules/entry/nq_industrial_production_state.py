...
class NqIndustrialProductionStateEntry:
    name = "nq_industrial_production_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "ipman_3m_weakness_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_industrial_production_state_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_industrial_production_state_{self.setup_mode}",
            ...
            metadata={
                ...
                "industrial_production_driver_column": driver_column,
                "industrial_production_driver_value": driver_value,
                "stop_pct": self.stop_pct,
                "target_r_multiple": self.target_r_multiple,
                "flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            },
            report_fields={
                "academic_source_key": "fama_1990_real_activity_fama_french_1989_business_conditions_fred_ipman",
                "setup_mode": self.setup_mode,
                "feature_csv": self.feature_csv,
                "feature_session_date": session_date.isoformat(),
                "industrial_production_observation_date": row.get("observation_date"),
                "observation_cutoff": row.get("observation_cutoff"),
                "availability_lag_days": row.get("availability_lag_days"),
                "availability_rule": "latest FRED IPMAN/INDPRO/CUMFNS monthly observation on or before NQ session_date minus 45 calendar days",
                ...
                "ipman": row.get("ipman"),
                "indpro": row.get("indpro"),
                "cumfns": row.get("cumfns"),
                "ipman_change_3m": row.get("ipman_change_3m"),
                "ipman_change_6m": row.get("ipman_change_6m"),
                "indpro_change_3m": row.get("indpro_change_3m"),
                "cumfns_rank_120m": row.get("cumfns_rank_120m"),
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "ipman_3m_weakness_long":
            column = "ipman_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "ipman_3m_strength_short":
            column = "ipman_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "ipman_6m_weakness_long":
            column = "ipman_change_6m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "indpro_3m_strength_long":
            column = "indpro_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "cumfns_3m_weakness_long":
            column = "cumfns_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        raise ValueError(
            f"Unsupported setup_mode for nq_industrial_production_state: {self.setup_mode}"
        )
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ industrial-production feature CSV not found: {path}")
