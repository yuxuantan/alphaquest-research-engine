...
class NqRetailInventoryDemandEntry:
    name = "nq_retail_inventory_demand"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "retail_3m_weakness_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_retail_inventory_demand_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_retail_inventory_demand_{self.setup_mode}",
            ...
            metadata={
                ...
                "retail_inventory_driver_column": driver_column,
                "retail_inventory_driver_value": driver_value,
                "stop_pct": self.stop_pct,
                "target_r_multiple": self.target_r_multiple,
                "flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            },
            report_fields={
                "academic_source_key": "lettau_ludvigson_2001_consumption_expected_returns_bils_kahn_2000_inventory_cycle_fred_retail_inventory",
                "setup_mode": self.setup_mode,
                "feature_csv": self.feature_csv,
                "feature_session_date": session_date.isoformat(),
                "retail_inventory_observation_date": row.get("observation_date"),
                "observation_cutoff": row.get("observation_cutoff"),
                "availability_lag_days": row.get("availability_lag_days"),
                "availability_rule": "latest FRED RSAFS/RSXFS/ISRATIO/BUSINV monthly observation on or before NQ session_date minus 45 calendar days",
                ...
                "retail_sales_change_1m": row.get("retail_sales_change_1m"),
                "retail_sales_ex_auto_change_3m": row.get("retail_sales_ex_auto_change_3m"),
                "inventory_sales_ratio": row.get("inventory_sales_ratio"),
                "inventory_sales_ratio_change_3m": row.get("inventory_sales_ratio_change_3m"),
                "business_inventories_change_3m": row.get("business_inventories_change_3m"),
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "retail_3m_weakness_long":
            column = "retail_sales_ex_auto_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "retail_3m_strength_short":
            column = "retail_sales_ex_auto_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "retail_1m_strength_long":
            column = "retail_sales_change_1m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "inventory_sales_high_short":
            column = "inventory_sales_ratio_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "inventory_sales_low_long":
            column = "inventory_sales_ratio_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "inventory_sales_falling_long":
            column = "inventory_sales_ratio_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        raise ValueError(f"Unsupported setup_mode for nq_retail_inventory_demand: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ retail/inventory demand feature CSV not found: {path}")
