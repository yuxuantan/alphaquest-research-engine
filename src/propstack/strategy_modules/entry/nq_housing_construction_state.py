...
class NqHousingConstructionStateEntry:
    name = "nq_housing_construction_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "permits_3m_strength_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_housing_construction_state_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_housing_construction_state_{self.setup_mode}",
            ...
            metadata={
                ...
                "housing_driver_column": driver_column,
                "housing_driver_value": driver_value,
                "stop_pct": self.stop_pct,
                "target_r_multiple": self.target_r_multiple,
                "flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            },
            report_fields={
                "academic_source_key": "census_fred_housing_construction_leading_state",
                "setup_mode": self.setup_mode,
                "feature_csv": self.feature_csv,
                "feature_session_date": session_date.isoformat(),
                "housing_observation_date": row.get("observation_date"),
                "observation_cutoff": row.get("observation_cutoff"),
                "availability_lag_days": row.get("availability_lag_days"),
                "availability_rule": "latest FRED/Census monthly housing observation on or before NQ session_date minus 45 calendar days",
                ...
                "single_family_permits": row.get("single_family_permits"),
                "total_starts": row.get("total_starts"),
                "permit_starts_ratio": row.get("permit_starts_ratio"),
                "total_permits_change_3m": row.get("total_permits_change_3m"),
                "total_starts_change_3m": row.get("total_starts_change_3m"),
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "permits_3m_strength_long":
            column = "total_permits_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "permits_3m_weakness_short":
            column = "total_permits_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "starts_3m_weakness_short":
            column = "total_starts_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "multifamily_starts_3m_weakness_short":
            column = "multifamily_starts_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "single_family_permits_strength_long":
            column = "single_family_permits_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "permit_starts_ratio_high_long":
            column = "permit_starts_ratio_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        raise ValueError(f"Unsupported setup_mode for nq_housing_construction_state: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ housing-construction feature CSV not found: {path}")
