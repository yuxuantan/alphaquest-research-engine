...
class CreditEtfOrderflowStateEntry:
    name = "credit_etf_orderflow_state"
    ...
    _RANK_COLUMNS = {
        "hyg_1d": "hyg_ret_1d_rank_252",
        "hyg_3d": "hyg_ret_3d_rank_252",
        "hyg_5d": "hyg_ret_5d_rank_252",
        "hyg_lqd_3d": "hyg_lqd_excess_3d_rank_252",
        "hyg_spy_1d": "hyg_spy_excess_1d_rank_252",
        "hyg_spy_3d": "hyg_spy_excess_3d_rank_252",
    }

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "hyg_strength_long")).lower()
        self.rank_mode = str(params.get("rank_mode", "hyg_1d")).lower()
        self.flow_mode = str(params.get("flow_mode", "signed_volume")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/es_credit_etf_risk_appetite_features_20110103_20260609.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "credit_market_lead_lag_hyg_orderflow",
            "setup_mode": self.setup_mode,
            "rank_mode": self.rank_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "credit_etf_observation_date": row.get("observation_date"),
            "availability_rule": row.get(
                "availability_rule", "latest ETF daily close strictly before ES session_date"
            ),
            "credit_driver_column": driver_column,
            "credit_driver_value": driver_value,
            "rank_threshold": self.rank_threshold,
            "hyg_ret_1d": row.get("hyg_ret_1d"),
            "hyg_ret_3d": row.get("hyg_ret_3d"),
            "hyg_ret_5d": row.get("hyg_ret_5d"),
            "hyg_ret_1d_rank_252": row.get("hyg_ret_1d_rank_252"),
            "hyg_ret_3d_rank_252": row.get("hyg_ret_3d_rank_252"),
            "hyg_ret_5d_rank_252": row.get("hyg_ret_5d_rank_252"),
            "hyg_lqd_excess_3d_rank_252": row.get("hyg_lqd_excess_3d_rank_252"),
            "hyg_spy_excess_1d_rank_252": row.get("hyg_spy_excess_1d_rank_252"),
        ...
        return Signal(
            direction=direction,
            level_type=f"credit_etf_orderflow_{self.rank_mode}_{self.setup_mode}_{direction}",
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"Credit ETF feature CSV not found: {path}")
