...
class MesCrowdingAoiTrapEntry:
    name = "mes_crowding_aoi_trap"
    ...
    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "market_aoi_trap")).lower()
    ...
    def _signal_from_completed_bar(self, bar: pd.Series) -> Signal | None:
        ...
        for direction, aoi_type, level in self._candidate_aois(bar):
            ...
            if direction == "short" and not self.allow_short:
                continue
            if not self._crowded_push_matches_direction(direction, metrics):
                continue
            if not self._trap_confirms(direction, bar, values, level):
                continue
            return self._signal(direction, aoi_type, level, bar, values, metrics, signal_timestamp)
        return None

    def _candidate_aois(self, bar: pd.Series) -> list[tuple[str, str, float]]:
        candidates: list[tuple[str, str, float]] = []
        mode = self.setup_mode
        if mode in {"prior_extreme_trap", "market_aoi_trap", "all_aoi_trap"}:
        ...
        if mode in {"opening_range_trap", "market_aoi_trap", "all_aoi_trap"}:
        ...
        if mode in {"overnight_trap", "market_aoi_trap", "all_aoi_trap"}:
        ...
        if mode in {"value_area_trap", "profile_aoi_trap", "all_aoi_trap"}:
        ...
        if mode in {"lvn_trap", "profile_aoi_trap", "all_aoi_trap"}:
            prefix = self.cached_profile_prefix
        ...
        if mode in {"profile_aoi_trap", "all_aoi_trap"}:
            prefix = self.cached_profile_prefix
    ...
    def _crowded_push_matches_direction(self, direction: str, metrics: dict[str, float]) -> bool:
        if direction == "long":
            return metrics["es_return_ticks"] <= -self.min_abs_return_ticks
        return metrics["es_return_ticks"] >= self.min_abs_return_ticks

    def _trap_confirms(self, direction: str, bar: pd.Series, values: dict[str, float], level: float) -> bool:
    ...
    def _signal(
        ...
        fields = {
            ...
            "footprint_absorption_long": bool(_truthy(bar.get("footprint_absorption_long"))),
            "footprint_absorption_short": bool(_truthy(bar.get("footprint_absorption_short"))),
            "signal_timestamp": signal_timestamp,
            "signal_close_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "signal_flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            "source_quality_label": "MES participation/crowding proxy plus completed-bar Sierra VAP/AOI/footprint fields",
        }
        return Signal(
            direction=direction,
            level_type=f"{aoi_type}_{self.share_mode}_mes_crowding_trap",
            swept_level=level,
    ...
    def _validate(self) -> None:
        ...
        if self.opening_range_minutes < 1 or self.lookback_minutes < 1 or self.rank_window < 1:
            raise ValueError("entry.params opening_range_minutes, lookback_minutes, and rank_window must be positive.")
        ...
        if self.min_abs_return_ticks < 0 or self.min_probe_ticks < 0 or self.confirmation_ticks < 0:
            raise ValueError("entry.params return, probe, and confirmation thresholds must be non-negative.")
        if self.min_delta_imbalance < 0:
