...
class NqSmallCapRelativeRotationEntry:
    name = "nq_small_cap_relative_rotation"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "iwm_1d_strength_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_small_cap_relative_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "lo_mackinlay_1990_size_lead_lag_chordia_sarkar_subrahmanyam_2005_size_liquidity_spillovers",
            "setup_mode": self.setup_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "small_cap_observation_date": row.get("observation_date"),
            "availability_cutoff": row.get("availability_cutoff"),
            "availability_lag_business_days": row.get("availability_lag_business_days"),
            "availability_rule": "latest IWM and QQQ daily close on or before NQ session_date minus one business day",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "small_cap_driver_column": driver_column,
            "small_cap_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "volume_rank_min": self.volume_rank_min,
            "iwm_minus_qqq_1d": row.get("iwm_minus_qqq_1d"),
            "iwm_minus_qqq_5d": row.get("iwm_minus_qqq_5d"),
            "iwm_volume_ratio_20": row.get("iwm_volume_ratio_20"),
            "iwm_attention_pressure_1d": row.get("iwm_attention_pressure_1d"),
            "iwm_minus_qqq_1d_rank_252": row.get("iwm_minus_qqq_1d_rank_252"),
            "iwm_minus_qqq_5d_rank_252": row.get("iwm_minus_qqq_5d_rank_252"),
            "iwm_volume_ratio_20_rank_252": row.get("iwm_volume_ratio_20_rank_252"),
            "iwm_attention_pressure_1d_rank_252": row.get(
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_small_cap_relative_rotation_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "iwm_1d_strength_long":
            column = "iwm_minus_qqq_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "iwm_5d_strength_long":
            column = "iwm_minus_qqq_5d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "iwm_1d_weakness_short":
            column = "iwm_minus_qqq_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "iwm_5d_weakness_short":
            column = "iwm_minus_qqq_5d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "iwm_attention_strength_long":
            strength_column = "iwm_attention_pressure_1d_rank_252"
            volume_column = "iwm_volume_ratio_20_rank_252"
        ...
        raise ValueError(
            f"Unsupported setup_mode for nq_small_cap_relative_rotation: {self.setup_mode}"
        )
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ small-cap relative feature CSV not found: {path}")
