...
class PreholidayEffectEntry:
    name = "preholiday_effect"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "unconditional_long")).lower()
        self.event_calendar_csv = str(
            params.get("event_calendar_csv", "data/external/nyse_preholiday_regular_sessions_20110103_20260609.csv")
        )
        self.signal_dates = _load_signal_dates(self.event_calendar_csv)
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction="long",
            level_type=f"preholiday_effect_{self.setup_mode}",
            ...
            report_fields={
                "academic_source_key": "fabozzi_ma_briley_1994_holiday_trading_futures",
                "setup_mode": self.setup_mode,
                "event_calendar_csv": self.event_calendar_csv,
                "preholiday_signal_session_date": session_date.isoformat(),
                "session_return_bps": session_return_bps,
                "session_range_bps": session_range_bps,
    ...
    def _passes_setup_filters(self, session_return_bps: float, session_range_bps: float) -> bool:
        ...
        raise ValueError(f"Unsupported setup_mode for preholiday_effect: {self.setup_mode}")
...
def _load_signal_dates(path: str) -> set[date]:
    csv_path = Path(path)
    if not csv_path.exists():
        raise FileNotFoundError(f"Pre-holiday calendar not found: {path}")
    out: set[date] = set()
