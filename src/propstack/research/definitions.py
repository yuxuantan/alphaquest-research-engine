...
DEFINITION_INDEX_SCHEMA = "propstack.definition-index/v1"
...
def definition_manifests(
    ...
    *,
    ...
    for campaign_dir in sorted(path for path in source_root.iterdir() if path.is_dir()):
        ...
        supplemental_configs = sorted(set(campaign_dir.rglob("config.yaml")) - set(original_configs) - rescue_configs)
        supplemental = []
        ...
        for attempt_index, (attempt_id, config_paths) in enumerate(sorted(attempts.items()), start=1):
            ...
            attempt_records.append({
                ...
                "attempt_kind": "rescue",
                "parent_attempt": "original",
                "authorization_status": "imported_from_legacy_authored_tree",
                "legacy_config_root": _display(root, rescue_root / attempt_id),
                "variants": references,
            })
