...
from propstack.backtest.metrics import daily_results
from propstack.data.databento_session_stream import DatabentoTradeSession
...
class ExactYushRangeBacktester:
    """Compatibility facade backed by :class:`BacktestEngine` event replay."""
    ...
    def __init__(
        ...
        self.config = config or ExactYushRangeConfig()
        self.news_events_by_session = news_events_by_session or {}

    def run(self, sessions: Iterable[DatabentoTradeSession]) -> dict:
        config = {
            "strategy_name": "yush_range_exact_databento",
            ...
            "core": {
                ...
                "tick_value": self.config.point_value * self.config.tick_size,
        ...
        result = BacktestEngine(config).run_event_replay(
            sessions,
            ExactYushRangeEventStrategy(self.config, self.news_events_by_session),
        )
