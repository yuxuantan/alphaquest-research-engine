...
class NqBtcCryptoRiskSentimentEntry:
    name = "nq_btc_crypto_risk_sentiment"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "btc_1d_strength_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_btc_crypto_risk_sentiment_features_20150115_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "imf_2022_crypto_stock_spillovers_imf_wp_2023_crypto_financial_spillovers_cme_2025_btc_nasdaq_correlation",
            "setup_mode": self.setup_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "btc_observation_date": row.get("observation_date"),
            "availability_cutoff": row.get("availability_cutoff"),
            "availability_lag_calendar_days": row.get("availability_lag_calendar_days"),
            "availability_rule": "latest BTC-USD daily close on or before NQ session_date minus one calendar day",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "btc_driver_column": driver_column,
            "btc_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "btc_return_1d": row.get("btc_return_1d"),
            "btc_return_3d": row.get("btc_return_3d"),
            "btc_return_7d": row.get("btc_return_7d"),
            "btc_abs_return_1d": row.get("btc_abs_return_1d"),
            "btc_return_1d_rank_252": row.get("btc_return_1d_rank_252"),
            "btc_return_3d_rank_252": row.get("btc_return_3d_rank_252"),
            "btc_abs_return_1d_rank_252": row.get("btc_abs_return_1d_rank_252"),
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_btc_crypto_risk_sentiment_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "btc_1d_strength_long":
            column = "btc_return_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "btc_3d_strength_long":
            column = "btc_return_3d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "btc_1d_weakness_short":
            column = "btc_return_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "btc_3d_weakness_short":
            column = "btc_return_3d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "btc_volatility_riskoff_short":
            column = "btc_abs_return_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        raise ValueError(f"Unsupported setup_mode for nq_btc_crypto_risk_sentiment: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ BTC risk-sentiment feature CSV not found: {path}")
