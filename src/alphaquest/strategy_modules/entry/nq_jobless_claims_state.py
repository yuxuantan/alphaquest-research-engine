...
class NqJoblessClaimsStateEntry:
    name = "nq_jobless_claims_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "claims_low_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_jobless_claims_state_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_jobless_claims_state_{self.setup_mode}",
            ...
            metadata={
                ...
                "jobless_claims_driver_column": driver_column,
                "jobless_claims_driver_value": driver_value,
                "stop_pct": self.stop_pct,
                "target_r_multiple": self.target_r_multiple,
                "flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            },
            report_fields={
                "academic_source_key": "fred_dol_weekly_jobless_claims_labor_market_state",
                "setup_mode": self.setup_mode,
                "feature_csv": self.feature_csv,
                "feature_session_date": session_date.isoformat(),
                "jobless_claims_observation_date": row.get("observation_date"),
                "observation_cutoff": row.get("observation_cutoff"),
                "availability_lag_days": row.get("availability_lag_days"),
                "availability_rule": "latest FRED/DOL weekly claims observation on or before NQ session_date minus 7 calendar days",
                ...
                "initial_claims_4w": row.get("initial_claims_4w"),
                "continued_claims_4w": row.get("continued_claims_4w"),
                "initial_claims_4w_change_1w": row.get("initial_claims_4w_change_1w"),
                "initial_claims_4w_change_4w": row.get("initial_claims_4w_change_4w"),
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "claims_low_long":
            column = "initial_claims_4w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "claims_high_short":
            column = "initial_claims_4w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "claims_rising_short":
            column = "initial_claims_4w_change_4w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "continued_claims_rising_short":
            column = "continued_claims_4w_change_1w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "claims_improving_long":
            column = "initial_claims_4w_change_1w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "continued_claims_improving_long":
            column = "continued_claims_4w_change_4w_rank_156w"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        raise ValueError(f"Unsupported setup_mode for nq_jobless_claims_state: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ jobless-claims feature CSV not found: {path}")
