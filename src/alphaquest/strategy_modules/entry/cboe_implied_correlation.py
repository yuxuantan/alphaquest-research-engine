...
class CboeImpliedCorrelationEntry:
    name = "cboe_implied_correlation"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "high_cor3m_short")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/es_cboe_implied_correlation_features_20110103_20260609.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        report_fields = {
            "academic_source_key": "cboe_implied_correlation_risk_premium",
        ...
        return Signal(
            direction=direction,
            level_type=f"cboe_implied_correlation_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "high_cor3m_short":
            ...
            return self._if_rank(column, rank, ">=", self.correlation_rank_min, "short")
        if self.setup_mode == "low_cor3m_long":
            column = "cor3m_close_rank_252"
            rank = _finite_float(row.get(column))
            return self._if_rank(column, rank, "<=", self.correlation_rank_max, "long")
        if self.setup_mode == "rising_cor3m_short":
            column = "cor3m_change_1d_rank_252"
            rank = _finite_float(row.get(column))
            return self._if_rank(column, rank, ">=", self.correlation_change_rank_min, "short")
        if self.setup_mode == "falling_cor3m_long":
            column = "cor3m_change_1d_rank_252"
            rank = _finite_float(row.get(column))
            return self._if_rank(column, rank, "<=", self.correlation_change_rank_max, "long")
        if self.setup_mode == "high_short_term_correlation_short":
            column = "cor1m_minus_cor3m_rank_252"
            rank = _finite_float(row.get(column))
            return self._if_rank(column, rank, ">=", self.term_spread_rank_min, "short")
        raise ValueError(f"Unsupported setup_mode for cboe_implied_correlation: {self.setup_mode}")
