...
class NqNasdaqEqualWeightConcentrationEntry:
    name = "nq_nasdaq_equal_weight_concentration"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "qqq_1d_concentration_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_nasdaq_equal_weight_features_20110103_20260612.csv",
            )
        )
        ...
        self.volume_rank_min = float(params.get("volume_rank_min", 0.50))
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "lo_mackinlay_1990_lead_lag_nasdaq_ndxe_methodology_equal_weight_concentration",
            "setup_mode": self.setup_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "nasdaq_equal_weight_observation_date": row.get("observation_date"),
            "availability_cutoff": row.get("availability_cutoff"),
            "availability_lag_business_days": row.get("availability_lag_business_days"),
            "availability_rule": "latest QQQ and QQQE daily close on or before NQ session_date minus one business day",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "concentration_driver_column": driver_column,
            "concentration_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "volume_rank_min": self.volume_rank_min,
            "qqq_minus_qqqe_1d": row.get("qqq_minus_qqqe_1d"),
            "qqq_minus_qqqe_5d": row.get("qqq_minus_qqqe_5d"),
            "qqq_volume_ratio_20": row.get("qqq_volume_ratio_20"),
            "concentration_pressure_1d": row.get("concentration_pressure_1d"),
            "qqq_minus_qqqe_1d_rank_252": row.get("qqq_minus_qqqe_1d_rank_252"),
            "qqq_minus_qqqe_5d_rank_252": row.get("qqq_minus_qqqe_5d_rank_252"),
            "qqq_volume_ratio_20_rank_252": row.get("qqq_volume_ratio_20_rank_252"),
            "concentration_pressure_1d_rank_252": row.get(
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_nasdaq_equal_weight_concentration_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "qqq_1d_concentration_long":
            column = "qqq_minus_qqqe_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "qqq_5d_concentration_long":
            column = "qqq_minus_qqqe_5d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "equal_weight_1d_breadth_short":
            column = "qqq_minus_qqqe_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "equal_weight_5d_breadth_short":
            column = "qqq_minus_qqqe_5d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "concentration_volume_pressure_long":
            strength_column = "concentration_pressure_1d_rank_252"
            volume_column = "qqq_volume_ratio_20_rank_252"
        ...
        raise ValueError(
            f"Unsupported setup_mode for nq_nasdaq_equal_weight_concentration: {self.setup_mode}"
        )
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ Nasdaq equal-weight concentration feature CSV not found: {path}")
