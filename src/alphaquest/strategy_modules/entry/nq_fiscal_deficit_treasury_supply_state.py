...
class NqFiscalDeficitTreasurySupplyStateEntry:
    name = "nq_fiscal_deficit_treasury_supply_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "high_deficit_3m_short")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_fiscal_deficit_treasury_supply_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "treasury_monthly_statement_fiscal_balance_supply_pressure",
            ...
            "availability_rule": "latest monthly Treasury/FRED MTS observation on or before session_date minus 60 calendar days",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "fiscal_driver_column": driver_column,
            "fiscal_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "fiscal_balance": row.get("fiscal_balance"),
            "federal_receipts": row.get("federal_receipts"),
            "federal_outlays": row.get("federal_outlays"),
            "deficit_3m_sum": row.get("deficit_3m_sum"),
            "deficit_12m_sum": row.get("deficit_12m_sum"),
            "receipts_yoy_growth": row.get("receipts_yoy_growth"),
            "outlays_yoy_growth": row.get("outlays_yoy_growth"),
            "fiscal_impulse_3m": row.get("fiscal_impulse_3m"),
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_fiscal_deficit_treasury_supply_state_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "high_deficit_3m_short":
            column = "deficit_3m_sum_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "high_deficit_12m_short":
            column = "deficit_12m_sum_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "strong_receipts_yoy_long":
            column = "receipts_yoy_growth_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "low_outlays_yoy_long":
            column = "outlays_yoy_growth_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "high_fiscal_impulse_short":
            column = "fiscal_impulse_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        raise ValueError(
            "Unsupported setup_mode for nq_fiscal_deficit_treasury_supply_state: "
            f"{self.setup_mode}"
        )
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ fiscal feature CSV not found: {path}")
