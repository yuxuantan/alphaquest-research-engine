...
class TradeOrderflowMultiStateRankEntry:
    name = "trade_orderflow_multi_state_rank"

    def __init__(self, params: dict):
        self.params = params
        slots = params.get("slots") or []
        if not slots:
            raise ValueError("trade_orderflow_multi_state_rank requires at least one slot.")
        self.max_trades_per_day = int(params.get("max_trades_per_day", len(slots)))
        self.slots = []
        for index, slot in enumerate(slots, start=1):
            slot = dict(slot or {})
            slot_params = {**params, **slot}
            ...
            slot_id = str(slot.get("slot_id", slot_params["setup_mode"]))
            entry = TradeOrderflowStateRankEntry(slot_params)
            self.slots.append((entry.entry_time, entry, slot_id))
