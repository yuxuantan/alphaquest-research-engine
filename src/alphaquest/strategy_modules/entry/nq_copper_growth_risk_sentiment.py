...
class NqCopperGrowthRiskSentimentEntry:
    name = "nq_copper_growth_risk_sentiment"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "copper_1d_strength_long")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_copper_growth_risk_sentiment_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "cme_copper_growth_indicator_parnes_2024_copper_gold_ratio_hu_xiong_2013_commodity_information",
            "setup_mode": self.setup_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "commodity_observation_date": row.get("observation_date"),
            "availability_cutoff": row.get("availability_cutoff"),
            "availability_lag_calendar_days": row.get("availability_lag_calendar_days"),
            "availability_rule": "latest copper/gold futures daily observation on or before NQ session_date minus one calendar day",
            "signal_timestamp": signal_timestamp,
            "intended_entry_timestamp": signal_timestamp,
            "copper_driver_column": driver_column,
            "copper_driver_value": driver_value,
            "rank_min": self.rank_min,
            "rank_max": self.rank_max,
            "copper_return_1d": row.get("copper_return_1d"),
            "copper_return_3d": row.get("copper_return_3d"),
            "copper_gold_ratio_return_3d": row.get("copper_gold_ratio_return_3d"),
            "copper_return_1d_rank_252": row.get("copper_return_1d_rank_252"),
            "copper_return_3d_rank_252": row.get("copper_return_3d_rank_252"),
            "copper_gold_ratio_return_3d_rank_252": row.get(
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_copper_growth_risk_sentiment_{self.setup_mode}",
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "copper_1d_strength_long":
            column = "copper_return_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "copper_3d_strength_long":
            column = "copper_return_3d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "copper_1d_weakness_short":
            column = "copper_return_1d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        if self.setup_mode == "copper_gold_ratio_strength_long":
            column = "copper_gold_ratio_return_3d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "long")
        if self.setup_mode == "copper_gold_ratio_weakness_short":
            column = "copper_gold_ratio_return_3d_rank_252"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "short")
        raise ValueError(
            f"Unsupported setup_mode for nq_copper_growth_risk_sentiment: {self.setup_mode}"
        )
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ copper growth/risk feature CSV not found: {path}")
