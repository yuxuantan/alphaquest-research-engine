...
class NqInflationPressureStateEntry:
    name = "nq_inflation_pressure_state"

    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "core_pce_high_short")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/nq_inflation_pressure_state_features_20110103_20260612.csv",
            )
        )
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        return Signal(
            direction=direction,
            level_type=f"nq_inflation_pressure_state_{self.setup_mode}",
            ...
            metadata={
                ...
                "inflation_driver_column": driver_column,
                "inflation_driver_value": driver_value,
                "stop_pct": self.stop_pct,
                "target_r_multiple": self.target_r_multiple,
                "flatten_time": self.flatten_time.strftime("%H:%M:%S"),
            },
            report_fields={
                "academic_source_key": "bea_bls_fred_inflation_pressure_state",
                "setup_mode": self.setup_mode,
                "feature_csv": self.feature_csv,
                "feature_session_date": session_date.isoformat(),
                "inflation_observation_date": row.get("observation_date"),
                "observation_cutoff": row.get("observation_cutoff"),
                "availability_lag_days": row.get("availability_lag_days"),
                "availability_rule": "latest FRED/BEA/BLS monthly inflation observation on or before NQ session_date minus 45 calendar days",
                ...
                "core_cpi_yoy": row.get("core_cpi_yoy"),
                "pce_3m_ann": row.get("pce_3m_ann"),
                "core_cpi_3m_ann": row.get("core_cpi_3m_ann"),
    ...
    def _signal_direction(self, row: dict[str, float | str]) -> tuple[str | None, str, float]:
        if self.setup_mode == "core_pce_high_short":
            column = "core_pce_yoy_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "core_pce_disinflation_long":
            column = "core_pce_yoy_change_3m_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        if self.setup_mode == "cpi_high_short":
            column = "cpi_yoy_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "core_cpi_accel_short":
            column = "core_cpi_3m_ann_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), ">=", self.rank_min, "short")
        if self.setup_mode == "pce_disinflation_long":
            column = "pce_3m_ann_rank_120m"
            return self._if_rank(column, _finite_float(row.get(column)), "<=", self.rank_max, "long")
        raise ValueError(f"Unsupported setup_mode for nq_inflation_pressure_state: {self.setup_mode}")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"NQ inflation-pressure feature CSV not found: {path}")
