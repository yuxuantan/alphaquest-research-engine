...
class SpyTurnoverOrderflowAttentionEntry:
    name = "spy_turnover_orderflow_attention"
    ...
    _RANK_COLUMNS = {
        "volume_20": "spy_volume_ratio_20_rank_252",
        "volume_63": "spy_volume_ratio_63_rank_252",
        "absret_1d": "spy_absret_volume_1d_rank_252",
        "absret_3d": "spy_absret_volume_3d_rank_252",
    }
    _RETURN_COLUMNS = {
        "spy_1d": "spy_ret_1d",
        "spy_3d": "spy_ret_3d",
    ...
    def __init__(self, params: dict):
        self.params = params
        self.setup_mode = str(params.get("setup_mode", "spy_two_sided_attention_continuation")).lower()
        self.rank_mode = str(params.get("rank_mode", "volume_20")).lower()
        self.return_mode = str(params.get("return_mode", "spy_1d")).lower()
        self.flow_mode = str(params.get("flow_mode", "signed_volume")).lower()
        self.feature_csv = str(
            params.get(
                "feature_csv",
                "data/external/es_spy_turnover_attention_features_20110103_20260609.csv",
            )
        )
        ...
        self.attention_rank_threshold = float(params.get("attention_rank_threshold", 0.60))
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0) -> Signal | None:
        ...
        if row is None:
            return None
        direction, rank_column, rank_value, return_column, return_value = self._direction_from_features(row, state)
        ...
        state["signaled"] = True
        report_fields = {
            "academic_source_key": "spy_turnover_attention_orderflow",
            "setup_mode": self.setup_mode,
            "rank_mode": self.rank_mode,
            "return_mode": self.return_mode,
            "feature_csv": self.feature_csv,
            "feature_session_date": session_date.isoformat(),
            "spy_observation_date": row.get("observation_date"),
            "availability_rule": row.get(
                "availability_rule", "latest SPY daily close and volume strictly before ES session_date"
            ),
            "attention_rank_column": rank_column,
            "attention_rank_value": rank_value,
            "attention_rank_threshold": self.attention_rank_threshold,
            "spy_return_column": return_column,
            "spy_return_value": return_value,
            "spy_volume_ratio_20": row.get("spy_volume_ratio_20"),
            "spy_volume_ratio_63": row.get("spy_volume_ratio_63"),
            "spy_absret_volume_1d": row.get("spy_absret_volume_1d"),
            "spy_absret_volume_3d": row.get("spy_absret_volume_3d"),
        ...
        return Signal(
            direction=direction,
            level_type=f"spy_turnover_attention_{self.rank_mode}_{self.return_mode}_{direction}",
            ...
            metadata={
                ...
                "rank_mode": self.rank_mode,
                "return_mode": self.return_mode,
                "attention_rank": rank_value,
                "spy_return": return_value,
                "flow_mode": self.flow_mode,
    ...
    def _direction_from_features(self, row: dict, state: dict) -> tuple[str | None, str, float, str, float]:
        move_ticks = _finite_float(state.get("move_ticks"))
        if move_ticks is None:
            return None, "", float("nan"), "", float("nan")
        rank_column = self._RANK_COLUMNS[self.rank_mode]
        return_column = self._RETURN_COLUMNS[self.return_mode]
        rank = _finite_float(row.get(rank_column))
        return_value = _finite_float(row.get(return_column))
        if rank is None or return_value is None:
            return None, rank_column, float("nan"), return_column, float("nan")
        if rank < self.attention_rank_threshold or return_value == 0:
            return None, rank_column, rank, return_column, return_value
        if self.setup_mode == "spy_up_attention_long":
            if return_value > 0 and move_ticks >= self.min_es_move_ticks:
                return "long", rank_column, rank, return_column, return_value
            return None, rank_column, rank, return_column, return_value
        if self.setup_mode == "spy_down_attention_short":
            if return_value < 0 and move_ticks <= -self.min_es_move_ticks:
                return "short", rank_column, rank, return_column, return_value
            return None, rank_column, rank, return_column, return_value
        if self.setup_mode == "spy_two_sided_attention_continuation":
            if return_value > 0 and move_ticks >= self.min_es_move_ticks:
                return "long", rank_column, rank, return_column, return_value
            if return_value < 0 and move_ticks <= -self.min_es_move_ticks:
                return "short", rank_column, rank, return_column, return_value
    ...
    def _validate(self) -> None:
        if self.setup_mode not in {
            "spy_up_attention_long",
            "spy_down_attention_short",
            "spy_two_sided_attention_continuation",
        }:
        ...
        if self.return_mode not in self._RETURN_COLUMNS:
            raise ValueError(f"entry.params.return_mode must be one of {sorted(self._RETURN_COLUMNS)}.")
        ...
        if not 0.5 <= self.attention_rank_threshold < 1.0:
            raise ValueError("entry.params.attention_rank_threshold must be in [0.5, 1.0).")
...
def _load_features(path: str) -> dict[date, dict[str, float | str]]:
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"SPY turnover feature CSV not found: {path}")
    out: dict[date, dict[str, float | str]] = {}
    string_columns = {"observation_date", "availability_rule"}
