...
_ORDERFLOW_PARAM_KEYS = {
    "orderflow_window_minutes",
    "flow_mode",
    "min_orderflow_imbalance",
    "consume_unconfirmed_base_signal",
}


class NqPivotMesOrderflowConfirmationEntry:
    name = "nq_pivot_mes_orderflow_confirmation"

    def __init__(self, params: dict):
        self.params = params
        market_structure_params = dict(params.get("market_structure_params") or params)
        for key in _ORDERFLOW_PARAM_KEYS:
            market_structure_params.pop(key, None)
        self.base_entry = MarketStructureFilteredEntry(market_structure_params)
        ...
        self.consume_unconfirmed_base_signal = bool(params.get("consume_unconfirmed_base_signal", True))
    ...
    def on_bar_close(self, bar: pd.Series, trades_today: int = 0):
        ...
        if signal is None:
            return None
        if state.get("consumed_unconfirmed", False):
            return None

        bar_close = timestamp + pd.Timedelta(minutes=self.bar_interval_minutes)
        observed = self._observed_values(state, bar_close, signal.direction)
        if not self._matches(observed):
            if self.consume_unconfirmed_base_signal:
                state["consumed_unconfirmed"] = True
            return None

        report_fields = {
            "nq_pivot_mes_orderflow_confirmation_result": "passed",
            "nq_orderflow_flow_mode": self.flow_mode,
            "nq_orderflow_window_minutes": self.orderflow_window_minutes,
            "nq_orderflow_min_imbalance": self.min_orderflow_imbalance,
            "nq_orderflow_consume_unconfirmed_base_signal": self.consume_unconfirmed_base_signal,
            **observed,
        }
        metadata = {
            **signal.metadata,
            "nq_orderflow_flow_mode": self.flow_mode,
            "nq_orderflow_window_minutes": self.orderflow_window_minutes,
            "nq_orderflow_signed_directional_imbalance": observed[
                "nq_orderflow_signed_directional_imbalance"
            ],
        }
        return replace(
            signal,
            level_type=f"{signal.level_type}_with_nq_orderflow_confirmation",
            metadata=metadata,
    ...
    def _flow_state(self, bar: pd.Series, timestamp: pd.Timestamp) -> dict:
        ...
        if state is None:
            state = {"bars": [], "consumed_unconfirmed": False}
    ...
    def _observed_values(self, state: dict, bar_close: pd.Timestamp, direction: str) -> dict:
        ...
        return {
            ...
            "nq_orderflow_volume_imbalance": _ratio(signed_volume, volume),
            "nq_orderflow_large10_imbalance": _ratio(large10_signed_volume, large10_volume),
            "nq_orderflow_large20_imbalance": _ratio(large20_signed_volume, large20_volume),
            "nq_orderflow_primary_imbalance": primary,
            "nq_orderflow_secondary_imbalance": secondary,
            "nq_orderflow_signed_directional_imbalance": signed_primary,
            "nq_orderflow_signed_secondary_directional_imbalance": signed_secondary,
        }
    ...
    def _matches(self, observed: dict) -> bool:
        primary = observed["nq_orderflow_signed_directional_imbalance"]
        secondary_raw = observed["nq_orderflow_secondary_imbalance"]
        secondary = observed["nq_orderflow_signed_secondary_directional_imbalance"]
