...
def _apply_mechanics_validation_contract(config: dict) -> None:
    ...
    if lane != "bar":
        raise ValueError(
            "generic mechanics validation supports only the bar lane; canonical event strategies must export "
            "event_transitions.parquet from their event-replay runner"
        )
    ...
    core["validation_export"] = {
        ...
        "window_bars_before": 10,
        "window_bars_after": 20,
        "max_trades": 30,
    }
...
def _validation_metadata(
    ...
    return ValidationMetadata(
        ...
        validation_lane="bar",
        source_data_type=(config.get("data") or {}).get("source"),
