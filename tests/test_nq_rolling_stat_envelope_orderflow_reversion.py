...
def test_nq_rolling_envelope_signal_uses_prior_completed_closes():
    ...
    assert signal.report_fields["feature_method"] == "prior_completed_rolling_close_envelope"
...
def test_nq_rolling_envelope_engine_enters_next_bar_open():
    timestamps = pd.date_range("2024-01-03 09:30:00", periods=7, freq="5min", tz="America/New_York")
    rows = [bar.to_dict() for bar in _lower_band_bars(timestamps=timestamps[:5])]
    ...
    cfg = {
        "strategy_name": "test_nq_rolling_stat_envelope_reversion",
        "timeframe": "5m",
        "strategy": {
            "entry": {
...
def _lower_band_bars(timestamps=None) -> list[pd.Series]:
...
def _bar(ts, idx, *, open_, high, low, close, signed_volume=0) -> pd.Series:
    return pd.Series(
        {
            "timestamp": ts,
