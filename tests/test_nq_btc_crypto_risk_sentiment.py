...
def test_entry_module_is_registered():
    assert (
        ENTRY_MODULES["nq_btc_crypto_risk_sentiment"]
        is NqBtcCryptoRiskSentimentEntry
    )


def test_btc_1d_strength_emits_long_on_completed_bar(tmp_path):
    features = _feature_file(tmp_path, "2024-04-03", btc_1d_rank=0.78)
    entry = NqBtcCryptoRiskSentimentEntry(
        {
            "feature_csv": str(features),
            "setup_mode": "btc_1d_strength_long",
            "rank_min": 0.65,
            "entry_time": "10:00:00",
    ...
    assert signal.report_fields["btc_driver_column"] == "btc_return_1d_rank_252"
    assert signal.report_fields["availability_rule"].startswith("latest BTC-USD")


def test_btc_3d_weakness_emits_short_only_below_rank_max(tmp_path):
    features = _feature_file(tmp_path, "2024-04-03", btc_3d_rank=0.46)
    entry = NqBtcCryptoRiskSentimentEntry(
        {
            "feature_csv": str(features),
            "setup_mode": "btc_3d_weakness_short",
            "rank_max": 0.35,
            "entry_time": "11:30:00",
        }
    )
    ...
    features = _feature_file(tmp_path, "2024-04-04", btc_3d_rank=0.22, name="weak.csv")
    entry = NqBtcCryptoRiskSentimentEntry(
        {
            "feature_csv": str(features),
            "setup_mode": "btc_3d_weakness_short",
...
def test_btc_volatility_riskoff_short_uses_abs_return_rank(tmp_path):
    features = _feature_file(tmp_path, "2024-04-03", btc_abs_rank=0.58)
    entry = NqBtcCryptoRiskSentimentEntry(
        {
            "feature_csv": str(features),
            "setup_mode": "btc_volatility_riskoff_short",
            "rank_min": 0.65,
            "entry_time": "13:30:00",
        }
    )
...
def test_builder_uses_one_calendar_day_availability_lag(tmp_path):
    ...
    _write_yahoo_csv(
        cache_dir / "yahoo_btc_usd_daily_2023-08-01_2024-05-31.csv",
        daily_dates,
        start_price=26000.0,
        volume=10000000000,
    )
    ...
    assert math.isfinite(later["btc_return_1d_rank_252"])
    assert math.isfinite(later["btc_abs_return_1d_rank_252"])
...
def _feature_file(
    ...
    session_date: str,
    *,
    btc_1d_rank: float = 0.5,
    btc_3d_rank: float = 0.5,
    btc_abs_rank: float = 0.5,
    name: str = "btc.csv",
):
    path = tmp_path / name
    path.write_text(
        "session_date,availability_cutoff,observation_date,availability_lag_calendar_days,"
        "btc,btc_volume,btc_return_1d,btc_return_3d,btc_return_7d,btc_abs_return_1d,"
        "btc_volume_ratio_20,btc_attention_pressure_1d,btc_return_1d_rank_252,"
        "btc_return_3d_rank_252,btc_return_7d_rank_252,btc_abs_return_1d_rank_252,"
        "btc_volume_ratio_20_rank_252,btc_attention_pressure_1d_rank_252\n"
        f"{session_date},2024-04-02,2024-04-02,1,65000,10000000000,"
        f"0.01,0.02,0.05,0.01,1.2,0.012,{btc_1d_rank},{btc_3d_rank},"
        f"0.5,{btc_abs_rank},0.5,0.5\n",
        encoding="utf-8",
    )
    return path
