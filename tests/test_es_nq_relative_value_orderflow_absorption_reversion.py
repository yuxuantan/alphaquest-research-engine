...
def test_es_nq_absorption_reversion_emits_long_when_es_underperforms_with_buy_absorption():
    entry = EsNqRelativeValueOrderflowAbsorptionReversionEntry(
        {
    ...
    signal = entry.on_bar_close(
        _bar("2024-01-03 09:59", es_return=-8.0, nq_return=1.0, es_signed_imbalance=0.18)
    )

    assert signal is not None
    assert signal.direction == "long"
    assert signal.report_fields["es_signed_absorption_imbalance"] == 0.18


def test_es_nq_absorption_reversion_emits_short_when_es_outperforms_with_sell_absorption():
    entry = EsNqRelativeValueOrderflowAbsorptionReversionEntry(
        {
            "setup_mode": "two_sided_divergence_fade",
            "entry_time": "10:00:00",
...
def test_es_nq_absorption_reversion_rejects_flow_that_confirms_the_es_price_move_and_releases_session():
    entry = EsNqRelativeValueOrderflowAbsorptionReversionEntry(
        {
            "setup_mode": "two_sided_divergence_fade",
            "entry_time": "10:00:00",
...
def test_engine_enters_es_nq_absorption_reversion_on_next_bar_open():
    ...
    df = pd.DataFrame(
        {
            ...
            "volume": [1000.0] * 4,
            "es_return_bps_15": [-8.0] * 4,
            "nq_return_bps_15": [1.0] * 4,
            "nq_minus_es_return_bps_15": [9.0] * 4,
            "es_signed_imbalance_15": [0.18] * 4,
            "es_return_bps_30": [-8.0] * 4,
            "nq_return_bps_30": [1.0] * 4,
            "nq_minus_es_return_bps_30": [9.0] * 4,
            "es_signed_imbalance_30": [0.18] * 4,
            "es_return_bps_60": [-8.0] * 4,
            "nq_return_bps_60": [1.0] * 4,
            "nq_minus_es_return_bps_60": [9.0] * 4,
            "es_signed_imbalance_60": [0.18] * 4,
        }
    )
    cfg = {
        "strategy_name": "test_es_nq_absorption_reversion",
        "timeframe": "1m",
        "strategy": {
            "entry": {
                "module": "es_nq_relative_value_orderflow_absorption_reversion",
                "params": {
                    "setup_mode": "two_sided_divergence_fade",
                    "entry_time": "10:00:00",
                    "bar_interval_minutes": 1,
    ...
    assert trade["es_signed_absorption_imbalance"] == 0.18


def _bar(timestamp, *, es_return: float, nq_return: float, es_signed_imbalance: float):
    ...
    return pd.Series(
        {
            ...
            f"es_signed_imbalance_{lookback}": es_signed_imbalance,
        }
    )
