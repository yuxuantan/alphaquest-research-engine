...
DEFAULT_OUTPUT = "data/external/nq_bank_credit_supply_state_features_20110103_20260612.csv"
DEFAULT_CACHE_DIR = "data/external/fred_bank_credit_supply_state"
...
SERIES = {
    ...
    "TOTLL": "loans_leases",
    "TOTCI": "ci_loans",
    "DPSACBW027SBOG": "deposits",
    "TLAACBW027SBOG": "total_assets",
}
...
def build_features(
    ...
    *,
    ...
    availability_lag_days: int = 14,
    rank_window_weeks: int = 260,
    rank_min_periods: int = 104,
    ...
    for fred_id, column in SERIES.items():
        ...
        weekly = series if weekly is None else weekly.merge(
            series, on="observation_date", how="inner", validate="one_to_one"
        )
    assert weekly is not None
    weekly = weekly.sort_values("observation_date", kind="mergesort").reset_index(drop=True)
    weekly = _add_state_features(
        weekly,
    ...
    merged = pd.merge_asof(
        ...
        weekly.sort_values("observation_date"),
    ...
    columns = [
        ...
        "deposits",
        "total_assets",
        "bank_credit_change_4w",
        "bank_credit_change_13w",
        "loans_leases_change_4w",
        "loans_leases_change_13w",
        "ci_loans_change_4w",
        "ci_loans_change_13w",
        "deposits_change_4w",
        "deposits_change_13w",
        "total_assets_change_4w",
        "total_assets_change_13w",
        "loan_to_asset",
        "ci_to_loans",
        "deposit_to_asset",
        "bank_credit_change_13w_rank_260w",
        "loans_leases_change_4w_rank_260w",
        "ci_loans_change_13w_rank_260w",
        "deposits_change_13w_rank_260w",
        "total_assets_change_4w_rank_260w",
        "loan_to_asset_rank_260w",
        "ci_to_loans_rank_260w",
        "deposit_to_asset_rank_260w",
    ]
    out = out[columns]
...
def _load_or_fetch_fred_series(
    ...
    cache_path = Path(cache_dir) / f"fred_{series_id.lower()}_weekly.csv"
...
def _add_state_features(
    weekly: pd.DataFrame,
    *,
    rank_window_weeks: int,
    rank_min_periods: int,
) -> pd.DataFrame:
    out = weekly.copy()
    for column in ["bank_credit", "loans_leases", "ci_loans", "deposits", "total_assets"]:
        out[f"{column}_change_4w"] = out[column].pct_change(4)
        out[f"{column}_change_13w"] = out[column].pct_change(13)
    out["loan_to_asset"] = out["loans_leases"] / out["total_assets"]
    out["ci_to_loans"] = out["ci_loans"] / out["loans_leases"]
    out["deposit_to_asset"] = out["deposits"] / out["total_assets"]
    for column in [
        "bank_credit_change_13w",
        "loans_leases_change_4w",
        "ci_loans_change_13w",
        ...
        out[f"{column}_rank_260w"] = _rolling_last_percentile(
            out[column],
            rank_window_weeks,
            rank_min_periods,
        )
...
def main() -> None:
    ...
    parser.add_argument("--availability-lag-days", type=int, default=14)
