...
FEATURE_CSV = Path("data/external/nq_credit_quality_stress_state_features_20110103_20260612.csv")
DETAIL_CSV = Path("research_artifacts/nq_credit_quality_stress_state_density_audit_20260701.csv")
SUMMARY_CSV = Path("research_artifacts/nq_credit_quality_stress_state_density_summary_20260701.csv")
REPORT_MD = Path("research_artifacts/nq_credit_quality_stress_state_density_audit_20260701.md")

RANK_THRESHOLDS = [0.50, 0.55, 0.60]
MIN_SIGNALS_PER_YEAR = 50.0

VARIANTS = [
    ("high_credit_card_chargeoff_short_1000", "credit_card_chargeoff_rank_80q", "high"),
    ("high_business_loan_chargeoff_short_1030", "business_loan_chargeoff_rank_80q", "high"),
    ("high_all_loan_chargeoff_short_1130", "all_loan_chargeoff_rank_80q", "high"),
    ("falling_credit_card_delinquency_long_1200", "credit_card_delinquency_change_4q_rank_80q", "low"),
    ("falling_consumer_loan_delinquency_long_1330", "consumer_loan_delinquency_change_4q_rank_80q", "low"),
]
WINDOWS = {
...
def audit() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rows = []
    for variant_id, rank_column, side in VARIANTS:
        ranks = pd.to_numeric(features[rank_column], errors="coerce")
        for rank_threshold in RANK_THRESHOLDS:
            if side == "high":
                mask = (ranks >= rank_threshold).fillna(False)
                rank_max_cutoff = None
            else:
                rank_max_cutoff = 1.0 - rank_threshold
                mask = (ranks <= rank_max_cutoff).fillna(False)
                ...
                rows.append(
                    {
                        ...
                        "rank_max_cutoff": rank_max_cutoff,
                        "window": window_name,
...
def _write_report(detail: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    lines = [
        "# NQ Credit Quality Stress State Density Audit",
        "",
        "Date: 2026-07-01",
        "",
        f"Verdict: {verdict}",
        "",
        "This is a pre-PnL opportunity-count audit for 120-day-lagged FRED bank delinquency and charge-off state.",
