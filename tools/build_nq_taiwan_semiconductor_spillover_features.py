...
DEFAULT_OUTPUT = "data/external/nq_taiwan_semiconductor_spillover_features_20110103_20260612.csv"
DEFAULT_CACHE_DIR = "data/external/yahoo_taiwan_semiconductor_spillover"
...
def build_features(
    ...
    *,
    ...
    twii = _load_or_fetch_yahoo("^TWII", cache_dir, yahoo_start_date, yahoo_end_date, "twii")
    tsmc = _load_or_fetch_yahoo("2330.TW", cache_dir, yahoo_start_date, yahoo_end_date, "tsmc_tw")
    daily = (
        twii.merge(tsmc, on="observation_date", how="inner", validate="one_to_one")
        .sort_values("observation_date", kind="mergesort")
        .reset_index(drop=True)
    )

    for symbol in ("twii", "tsmc_tw"):
        for lookback in (1, 3, 5):
            daily[f"{symbol}_return_{lookback}d"] = daily[symbol].pct_change(lookback)
    for lookback in (1, 3, 5):
        daily[f"tsmc_tw_twii_relative_return_{lookback}d"] = (
            daily[f"tsmc_tw_return_{lookback}d"] - daily[f"twii_return_{lookback}d"]
        )
    daily["twii_abs_return_1d"] = daily["twii_return_1d"].abs()
    ...
    rank_columns = [
        ...
        "tsmc_tw_twii_relative_return_5d",
    ]
    ...
    merged["taiwan_observation_lag_calendar_days"] = (
    ...
    columns = [
        "session_date",
        "observation_date",
        "taiwan_observation_lag_calendar_days",
        "twii",
        "tsmc_tw",
        "twii_return_1d",
        "twii_return_3d",
        ...
        "tsmc_tw_twii_relative_return_3d",
        "tsmc_tw_twii_relative_return_5d",
        "twii_return_1d_rank_252",
        "twii_return_3d_rank_252",
        "twii_return_5d_rank_252",
        "twii_abs_return_1d_rank_252",
