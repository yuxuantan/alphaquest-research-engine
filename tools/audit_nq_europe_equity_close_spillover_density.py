...
DEFAULT_FEATURES = "data/external/nq_europe_equity_close_spillover_features_20110103_20260612.csv"
DEFAULT_DETAIL = "research_artifacts/nq_europe_equity_close_spillover_density_audit_20260701.csv"
DEFAULT_SUMMARY = "research_artifacts/nq_europe_equity_close_spillover_density_summary_20260701.csv"
DEFAULT_MD = "research_artifacts/nq_europe_equity_close_spillover_density_audit_20260701.md"
...
VARIANTS = [
    Variant(
        "dax_1d_strength_long_1330",
        "dax_1d_strength_long",
        "long",
        "rank_min",
        (0.60, 0.65, 0.70),
    ...
    Variant(
        "stoxx_1d_strength_long_1400",
        "stoxx_1d_strength_long",
        "long",
        "rank_min",
        (0.60, 0.65, 0.70),
    ...
    Variant(
        "dax_1d_weakness_short_1330",
        "dax_1d_weakness_short",
        "short",
        "rank_max",
        (0.30, 0.35, 0.40),
        "dax_return_1d_rank_252",
    ...
    Variant(
        "stoxx_1d_weakness_short_1400",
        "stoxx_1d_weakness_short",
        "short",
        "rank_max",
        (0.30, 0.35, 0.40),
        "stoxx50_return_1d_rank_252",
    ...
    Variant(
        "europe_broad_weakness_short_1500",
        "europe_broad_weakness_short",
        "short",
        "rank_max",
        (0.40, 0.45, 0.50),
        "europe_broad_weakness_min_rank",
    ),
]
...
WINDOWS = {
    ...
    "limited_core_proxy": ("2011-02-22", "2012-09-07"),
...
def audit(
    ...
    *,
    ...
    features["same_day_europe_close_available"] = features[
    ...
    passing_variants = int(summary["variant_pass"].sum())
    md_lines = [
        "# NQ Europe Equity Close Spillover Density Audit",
        "",
        ...
        f"- Density pass rows: {pass_rows}/{total_rows}",
        f"- Passing variants: {passing_variants}/{len(summary)}",
...
def _signal_mask(features: pd.DataFrame, variant: Variant, threshold: float) -> pd.Series:
    available = features["same_day_europe_close_available"].fillna(False)
    if variant.setup_mode == "europe_broad_weakness_short":
        return (
            available
            & (features["dax_return_1d_rank_252"] <= threshold)
            & (features["stoxx50_return_1d_rank_252"] <= threshold)
        )
    if variant.direction == "long":
        return available & (features[variant.rank_column] >= threshold)
    return available & (features[variant.rank_column] <= threshold)
