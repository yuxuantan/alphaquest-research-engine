...
DEFAULT_OUTPUT = "data/external/nq_sloos_bank_lending_survey_state_features_20110103_20260612.csv"
DEFAULT_CACHE_DIR = "data/external/fred_sloos_bank_lending_survey_state"
FRED_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id={series_id}"

SERIES = {
    "DRTSCILM": "ci_large_tightening",
    "DRTSCIS": "ci_small_tightening",
    "DRSDCILM": "ci_large_demand",
    "DRSDCIS": "ci_small_demand",
    "DRTSCLCC": "credit_card_tightening",
}
...
def build_features(
    ...
    *,
    ...
    columns = [
        ...
        "ci_large_demand",
        "ci_small_demand",
        "credit_card_tightening",
        "ci_large_tightening_change_1q",
        "ci_large_tightening_change_4q",
        "ci_small_tightening_change_1q",
        "ci_small_tightening_change_4q",
        "ci_large_demand_change_1q",
        "ci_large_demand_change_4q",
        "ci_small_demand_change_1q",
        "ci_small_demand_change_4q",
        "credit_card_tightening_change_1q",
        "credit_card_tightening_change_4q",
        "ci_large_tightening_rank_80q",
        "ci_small_tightening_rank_80q",
        "ci_large_demand_rank_80q",
        "ci_small_demand_rank_80q",
        "credit_card_tightening_rank_80q",
        "ci_demand_rank_average_80q",
        "ci_demand_rank_max_80q",
        "ci_demand_rank_min_80q",
    ]
    out = out[columns]
...
def _add_state_features(
    ...
    *,
    ...
    for column in columns:
        out[f"{column}_change_1q"] = out[column].diff(1)
        out[f"{column}_change_4q"] = out[column].diff(4)
        out[f"{column}_rank_80q"] = _rolling_last_percentile(
            out[column],
            rank_window_quarters,
            rank_min_periods,
        )
    out["ci_demand_rank_average_80q"] = (
        out["ci_large_demand_rank_80q"] + out["ci_small_demand_rank_80q"]
    ) / 2.0
    out["ci_demand_rank_max_80q"] = out[
        ["ci_large_demand_rank_80q", "ci_small_demand_rank_80q"]
    ].max(axis=1)
    out["ci_demand_rank_min_80q"] = out[
        ["ci_large_demand_rank_80q", "ci_small_demand_rank_80q"]
    ].min(axis=1)
    return out
...
def _read_csv_with_retries(url: str, attempts: int = 4, sleep_seconds: float = 2.0) -> pd.DataFrame:
    ...
    for attempt in range(1, attempts + 1):
        ...
        except Exception as exc:  # pragma: no cover - external fallback.
            last_error = exc
        try:
...
def main() -> None:
    ...
    parser.add_argument("--availability-lag-days", type=int, default=75)
