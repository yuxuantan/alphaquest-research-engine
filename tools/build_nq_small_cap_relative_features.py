...
DEFAULT_OUTPUT = "data/external/nq_small_cap_relative_features_20110103_20260612.csv"
DEFAULT_CACHE_DIR = "data/external/yahoo_size_etfs"
...
def build_features(
    ...
    *,
    ...
    iwm = _load_or_fetch_yahoo("IWM", cache_dir, yahoo_start_date, yahoo_end_date, "iwm")
    qqq = _load_or_fetch_yahoo("QQQ", cache_dir, yahoo_start_date, yahoo_end_date, "qqq")
    daily = (
        iwm.merge(qqq, on="observation_date", how="inner")
        .sort_values("observation_date", kind="mergesort")
        .reset_index(drop=True)
    )
    for symbol in ("iwm", "qqq"):
        for lookback in (1, 5):
            daily[f"{symbol}_return_{lookback}d"] = daily[symbol].pct_change(lookback)
    daily["iwm_minus_qqq_1d"] = daily["iwm_return_1d"] - daily["qqq_return_1d"]
    daily["iwm_minus_qqq_5d"] = daily["iwm_return_5d"] - daily["qqq_return_5d"]
    daily["iwm_volume_median_20"] = daily["iwm_volume"].rolling(20, min_periods=10).median()
    daily["iwm_volume_ratio_20"] = daily["iwm_volume"] / daily["iwm_volume_median_20"]
    daily["iwm_attention_pressure_1d"] = daily["iwm_minus_qqq_1d"] * daily[
        "iwm_volume_ratio_20"
    ]
    ...
    columns = [
        ...
        "iwm_volume_ratio_20",
        "iwm_attention_pressure_1d",
        "iwm_minus_qqq_1d_rank_252",
        "iwm_minus_qqq_5d_rank_252",
        "iwm_volume_ratio_20_rank_252",
...
def main() -> None:
    ...
    valid = features.dropna(
        subset=["iwm_minus_qqq_1d_rank_252", "iwm_minus_qqq_5d_rank_252"]
    )
