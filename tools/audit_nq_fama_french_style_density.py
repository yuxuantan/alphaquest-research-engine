...
DEFAULT_FEATURES = "data/external/nq_fama_french_style_features_20110103_20260612.csv"
DEFAULT_DETAIL = "research_artifacts/nq_fama_french_style_factor_rotation_density_audit_20260701.csv"
DEFAULT_SUMMARY = "research_artifacts/nq_fama_french_style_factor_rotation_density_summary_20260701.csv"
DEFAULT_MARKDOWN = "research_artifacts/nq_fama_french_style_factor_rotation_density_audit_20260701.md"


VARIANT_GRID = {
    "hml_21d_value_strength_short_1000": [
        {"column": "hml_21d_rank_252", "tail": "high", "factor_rank_threshold": value}
        for value in (0.25, 0.30, 0.35)
    ],
    "hml_21d_growth_strength_long_1030": [
        {"column": "hml_21d_rank_252", "tail": "low", "factor_rank_threshold": value}
        for value in (0.30, 0.35, 0.40)
    ],
    "rmw_63d_quality_strength_long_1130": [
        {"column": "rmw_63d_rank_252", "tail": "high", "factor_rank_threshold": value}
        for value in (0.25, 0.30, 0.35)
    ],
    "cma_63d_conservative_short_1200": [
        {"column": "cma_63d_rank_252", "tail": "high", "factor_rank_threshold": value}
        for value in (0.25, 0.30, 0.35)
    ],
    "hml_63d_extreme_two_sided_1330": [
        {"column": "hml_63d_rank_252", "tail": "two_sided", "factor_rank_threshold": value}
        for value in (0.25, 0.30, 0.35)
    ],
}
            ...
            rows.append(
                {
                    ...
                    "tail": spec["tail"],
                    "factor_rank_threshold": spec["factor_rank_threshold"],
...
def _mask(frame: pd.DataFrame, spec: dict) -> pd.Series:
    ...
    threshold = float(spec["factor_rank_threshold"])
    tail = spec["tail"]
    if tail == "high":
        mask = series >= 1.0 - threshold
    elif tail == "low":
        mask = series <= threshold
    elif tail == "two_sided":
        mask = (series <= threshold) | (series >= 1.0 - threshold)
    else:
