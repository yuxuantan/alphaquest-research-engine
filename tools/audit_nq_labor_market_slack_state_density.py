...
FEATURE_CSV = Path("data/external/nq_labor_market_slack_state_features_20110103_20260612.csv")
DETAIL_CSV = Path("research_artifacts/nq_labor_market_slack_state_density_audit_20260702.csv")
SUMMARY_CSV = Path("research_artifacts/nq_labor_market_slack_state_density_summary_20260702.csv")
REPORT_MD = Path("research_artifacts/nq_labor_market_slack_state_density_audit_20260702.md")

RANK_THRESHOLDS = [0.50, 0.55, 0.60]
MIN_SIGNALS_PER_YEAR = 50.0

VARIANTS = [
    ("high_unemployment_slack_short_1000", "unemployment_rate_rank_120m", "high"),
    ("high_underemployment_slack_short_1030", "underemployment_rate_rank_120m", "high"),
    ("low_employment_ratio_slack_short_1130", "employment_population_ratio_rank_120m", "low"),
    ("low_participation_slack_short_1200", "participation_rate_rank_120m", "low"),
    ("rising_participation_repair_long_1330", "participation_rate_change_3m_rank_120m", "high"),
]
WINDOWS = {
...
def audit() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for variant_id, rank_column, side in VARIANTS:
        ...
        for rank_threshold in RANK_THRESHOLDS:
            if side == "high":
                cutoff = rank_threshold
                mask = (ranks >= cutoff).fillna(False)
            else:
                cutoff = 1.0 - rank_threshold
                mask = (ranks <= cutoff).fillna(False)
...
def _write_report(detail: pd.DataFrame, summary: pd.DataFrame) -> None:
    ...
    lines = [
        "# NQ Labor Market Slack State Density Audit",
        "",
        "Date: 2026-07-02",
        "",
        f"Verdict: {verdict}",
        "",
        "This is a pre-PnL opportunity-count audit for 45-day-lagged FRED/BLS labor-market slack and participation state.",
