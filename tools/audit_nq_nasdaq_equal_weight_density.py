...
DEFAULT_FEATURES = "data/external/nq_nasdaq_equal_weight_features_20110103_20260612.csv"
DEFAULT_DETAIL = "research_artifacts/nq_nasdaq_equal_weight_concentration_density_audit_20260701.csv"
DEFAULT_SUMMARY = "research_artifacts/nq_nasdaq_equal_weight_concentration_density_summary_20260701.csv"
DEFAULT_MARKDOWN = "research_artifacts/nq_nasdaq_equal_weight_concentration_density_audit_20260701.md"


VARIANT_GRID = {
    "qqq_1d_concentration_long_1000": [
        {"column": "qqq_minus_qqqe_1d_rank_252", "op": ">=", "rank_min": value}
        for value in (0.55, 0.60, 0.65)
    ],
    "qqq_5d_concentration_long_1030": [
        {"column": "qqq_minus_qqqe_5d_rank_252", "op": ">=", "rank_min": value}
        for value in (0.55, 0.60, 0.65)
    ],
    "equal_weight_1d_breadth_short_1000": [
        {"column": "qqq_minus_qqqe_1d_rank_252", "op": "<=", "rank_max": value}
        for value in (0.45, 0.40, 0.35)
    ],
    "equal_weight_5d_breadth_short_1130": [
        {"column": "qqq_minus_qqqe_5d_rank_252", "op": "<=", "rank_max": value}
        for value in (0.45, 0.40, 0.35)
    ],
    "concentration_volume_pressure_long_1330": [
        {
            "column": "concentration_pressure_1d_rank_252",
            "op": ">=",
            "rank_min": rank_min,
            "volume_rank_min": volume_rank_min,
        }
        for rank_min in (0.40, 0.45, 0.50)
        for volume_rank_min in (0.40, 0.45, 0.50)
    ],
}
...
def audit_density(
    ...
    *,
            ...
            rows.append(
                {
                    ...
                    "op": spec["op"],
                    "rank_min": spec.get("rank_min"),
                    "rank_max": spec.get("rank_max"),
                    "volume_rank_min": spec.get("volume_rank_min"),
    ...
    summary["variant_pass_density"] = (
        summary["declared_entry_rows"] == summary["passing_entry_rows"]
    )
    ...
    _write_markdown(
        ...
        min_annual_signals=min_annual_signals,
        min_latest_252_signals=min_latest_252_signals,
        limited_start=limited_start,
        limited_end=limited_end,
    )
...
def _mask(frame: pd.DataFrame, spec: dict) -> pd.Series:
    ...
    if spec["op"] == ">=":
        mask = series >= float(spec["rank_min"])
    elif spec["op"] == "<=":
        mask = series <= float(spec["rank_max"])
    else:
        raise ValueError(f"Unsupported operator: {spec['op']}")
    if "volume_rank_min" in spec:
        volume = pd.to_numeric(frame["qqq_volume_ratio_20_rank_252"], errors="coerce")
        mask = mask & (volume >= float(spec["volume_rank_min"]))
...
def _write_markdown(
    ...
    *,
    ...
    min_annual_signals: float,
    min_latest_252_signals: int,
    limited_start: str,
    limited_end: str,
) -> None:
    ...
    lines = [
        ...
        f"Annualized full/limited-window threshold: {min_annual_signals:.2f} signals/year.",
        f"Latest-252-session threshold: {min_latest_252_signals} signals.",
        f"Limited-core proxy window: {limited_start} through {limited_end}.",
        "",
