# midpoint_low_sweep_reclaim_long

Campaign: `es_overnight_inventory_sweep_reversion`

## Mechanic
From 09:30 through 12:00 ET, record a completed 5-minute sweep below the completed overnight low, then require a later completed bar within the reclaim window to close above the overnight midpoint. Enter long at the next bar open; flatten at 15:55 ET unless stop or target is hit. The overnight high and low are completed before the RTH signal window begins.
...
User-authorized additional rescue created on 2026-06-19. Source run: `es_overnight_inventory_sweep_reversion/midpoint_low_sweep_reclaim_long/run1`. Only stop distance was widened by 1.5x; entry, target/exit, data, costs, fills, sessions, and validation gates are unchanged.
