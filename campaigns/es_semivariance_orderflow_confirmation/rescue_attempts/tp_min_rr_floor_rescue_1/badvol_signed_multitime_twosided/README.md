...
Original profitable rows were concentrated in wider-stop, 1R exits; rescue narrows the target grid to the plausible neighborhood while preserving the two-sided semivariance confirmation mechanic.
...
User-authorized additional rescue created on 2026-06-19. Source run: `es_semivariance_orderflow_confirmation/badvol_signed_multitime_twosided/rescue1`. Only target_r_multiple values below 1.0R were raised to 1.0R; entry, stop, data, costs, fills, sessions, and validation gates are unchanged.
