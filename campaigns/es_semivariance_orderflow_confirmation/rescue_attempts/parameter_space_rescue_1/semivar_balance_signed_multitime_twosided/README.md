# Semivariance-balance signed-flow multi-time two-sided

Campaign: `es_semivariance_orderflow_confirmation`
Variant: `semivar_balance_signed_multitime_twosided`

## Mechanics

Use the prior downside-minus-upside semivariance balance as a two-sided state, then require current-session price movement and completed 90-minute signed-flow confirmation at fixed decision times.
...
A balance measure should isolate whether recent realized risk was dominated by downside or upside variation. The longer flow window reduces single-burst noise and only trades when the current session confirms the lagged balance state.
...
Original semivariance-balance rows were negative but least bad at larger targets; rescue tests the adjacent target band once as required, with no filter or mechanic changes.
