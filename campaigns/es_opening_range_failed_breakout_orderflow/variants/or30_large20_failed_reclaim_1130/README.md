# or30_large20_failed_reclaim_1130

Campaign: `es_opening_range_failed_breakout_orderflow`

Mechanic: build the completed first 30-minute RTH opening range, require a completed close outside the range, then trade the completed reclaim back through the same boundary before 11:30:00 only when the reclaim bar has opposite large20 signed-volume imbalance.
...
Parameter grid: `entry.params.max_reclaim_bars` x `entry.params.min_reclaim_orderflow_imbalance` x `sl.params.stop_offset_ticks` = 27 combinations.
