...
Mechanic: build the completed first 15-minute RTH opening range, require a completed close outside the range, then trade the completed reclaim back through the same boundary before 10:30:00 only when the reclaim bar has opposite large10 signed-volume imbalance.
...
Parameter grid: `entry.params.max_reclaim_bars` x `entry.params.min_reclaim_orderflow_imbalance` x `sl.params.stop_offset_ticks` = 18 combinations.
