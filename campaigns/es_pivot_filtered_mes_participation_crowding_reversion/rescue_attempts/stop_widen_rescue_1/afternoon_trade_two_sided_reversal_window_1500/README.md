# afternoon_trade_two_sided_reversal_window_1500 stop_widen_rescue_1

Parameter-only rescue for `afternoon_trade_two_sided_reversal_window_1500` after original staged validation failed. Entry mechanics, signal window, carried pivot filter, data windows, costs, and target grid are unchanged. The stop grid changes from `[0.0015, 0.0025, 0.004]` to `[0.0025, 0.004, 0.006]`, and the fixed-config stop changes from `0.0025` to `0.004`.
