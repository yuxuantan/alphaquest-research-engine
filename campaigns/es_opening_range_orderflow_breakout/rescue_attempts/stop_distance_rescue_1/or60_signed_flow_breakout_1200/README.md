...
Mechanic: build the completed first 60-minute RTH opening range, then allow one two-sided breakout entry before 12:00 ET only when the completed 5-minute breakout bar closes beyond the range and has aligned aggregate signed-volume imbalance.
...
User-authorized additional rescue created on 2026-06-19. Source run: `es_opening_range_orderflow_breakout/or60_signed_flow_breakout_1200/run1`. Only stop distance was widened by 1.5x; entry, target/exit, data, costs, fills, sessions, and validation gates are unchanged.
