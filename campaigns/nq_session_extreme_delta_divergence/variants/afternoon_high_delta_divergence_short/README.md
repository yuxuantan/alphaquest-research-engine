...
Variant expression: Afternoon fresh-session-high signed-volume non-confirmation short.

Mechanic: From 13:00 through 15:15 ET, short NQ when a completed one-minute bar makes a fresh current-session high by the grid threshold, closes back within eight ticks of the prior completed high, and cumulative signed-volume progress since that prior high is weak. The reference high/low is from completed prior RTH bars only; the signal bar is evaluated after close and the staged engine enters no earlier than the next bar.
